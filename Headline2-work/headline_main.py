...
def main():
	...
	for test in tests:
		...
		for i in range(0,len(test.encStrs)):
			...
			if headlines[i].failed == True:
				diffEncStrs.append(test.encStrs[i])
				diffClrStrs.append(test.clrStrs[i])
	

	pickle.dump(diffEncStrs,open("DifficultEncodedStrings.p","wb"))
	pickle.dump(diffClrStrs,open("DifficultClearStrings.p","wb"))
	'''
